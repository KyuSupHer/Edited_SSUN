...
##SSUN
# define LSTM
# W:weight (hidden to hide) U:( input to hidden)
def LSTMMCNN_RS(time_step,nb_features,num_PC,img_rows,img_cols):
    ...
    #SSUN Model(joint+spectral+spatial)
    model = Model(input=[LSTMInput,CNNInput], output=[JOINTSOFTMAX,LSTMSOFTMAX,CNNSOFTMAX])    

    ##optimizerc
    #learning rate; float number(>=0); litte float number(Prevents division by 0 errors)
